  ...
  public static refreshData() {
    ...
    if (DataService.dataYaml) {
      try {
        ...
        console.error(e);
        vscode.window.showErrorMessage("Line: " + e.source.rawValue);
        vscode.window.showErrorMessage(
          "ERROR at line " + e.source.rangeAsLinePos.start.line + ", col " + e.source.rangeAsLinePos.start.col
        );
        vscode.window.showErrorMessage('File "openapi.yaml" not parsable: ' + e.message);
        ...
        try {
          ...
          docProm.then(doc => {
            ...
            var range: vscode.Range = new vscode.Range(
              e.source.rangeAsLinePos.start.line - 1,
              e.source.rangeAsLinePos.start.col - 1,
              e.source.rangeAsLinePos.end.line - 1,
              e.source.rangeAsLinePos.end.col - 1
            );
            tab.then(tab => {
              // Select range
