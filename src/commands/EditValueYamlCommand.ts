...
export class EditValueYamlCommand {
  static async command(context: SkaffolderNode) {
    if (context.params && context.params.range) {
      ...
      try {
        // Execute Command
        await vscode.commands.executeCommand<vscode.Location[]>("vscode.open", contexturl);
      } catch (e) {
        console.error(e);
      }

      // Select range
      let selection: vscode.Selection = new vscode.Selection(context.params.range.start, context.params.range.end);
